...
//	- 축구공: 3000원
	...
	public static void main(String[] args) {
		...
		HashSet<Equipment> union = new HashSet<Equipment>();
		HashSet<Equipment> intersection = new HashSet<Equipment>();
		HashSet<Equipment> difference = new HashSet<Equipment>();
		
		union.addAll(storage1);
		union.addAll(storage2);
		
		intersection.addAll(storage1);
		intersection.retainAll(storage2);
		
		difference.addAll(storage1);
		difference.removeAll(storage2);
		
		System.out.println("*** 합집합 ***");
		var it = union.iterator();
		while(it.hasNext())
