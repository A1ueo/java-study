...
//		set.add("JAVA");
		...
		// HashSet처럼 순서가 없는 컬렉션 내부의 요소를 순차저긍로 접근할 수 있도록 도와줌
		Iterator<String> iterator = set.iterator(); // 반복자 얻기(Iterator 구현 객체)
//		var it = set.iterator();
		
		while(iterator.hasNext()) { // 가져올 객체가 있으면 true, 없으면 fasle
...
//			iterator.remove();
			// 참고: remove는 next() 호출 이후에만 호출 가능하며, 한 번만 가능
		}
...
//		System.out.println("합집합: " + class1); // 합집합: [1, 2, 3, 4]
		
		// 교집합
		// 2개의 HashSet에서 공통된 요소만 추려줌
//		class1.retainAll(class2);
//		System.out.println("교집합: " + class1); // 교집합: [2, 3]
		
		// 차집합
		// 기준 HashSet에서 대상 HashSet과의 교집합 제외
//		class1.removeAll(class2);
//		System.out.println("차집합: " + class1); // 차집합: [1]
		
		// 주의: 3개의 메소드 모두 사용하면 기준이 되는 HashSt의 데이터 자체를 바꿔버림
